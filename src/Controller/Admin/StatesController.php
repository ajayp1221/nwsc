...
class StatesController extends AppController
{
    ...
    {
        $state =$this->States->find()->where(['States.slug' => $slug])->Contain(['Countries'])->first();
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
            $d = $this->request->data;
            $d['status'] = 1;
            $state = $this->States->patchEntity($state, $d);
    ...
    {
        $state =$this->States->find()->where(['States.slug' => $slug])->first();
