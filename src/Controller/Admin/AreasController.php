...
class AreasController extends AppController
{
    ...
    {
        $area = $this->Areas->find()->contain(['Cities'])->where(['Areas.slug'=>$slug])->first();
        $this->set('area', $area);
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
            $d = $this->request->data;
            $d['status'] = 1;
            $area = $this->Areas->patchEntity($area, $d);
        ...
        $cities = $this->Areas->Cities->find('list', ['limit' => 200]);
    ...
    {
        $area = $this->Areas->find()->where(['slug'=>$slug])->first();
