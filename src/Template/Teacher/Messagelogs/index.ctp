<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('New Messagelog'), ['action' => 'add']) ?></li>
        <li><?= $this->Html->link(__('List Messagesmslogs'), ['controller' => 'Messagesmslogs', 'action' => 'index']) ?></li>
        <li><?= $this->Html->link(__('New Messagesmslog'), ['controller' => 'Messagesmslogs', 'action' => 'add']) ?></li>
    </ul>
</nav>
<div class="messagelogs index large-9 medium-8 columns content">
    <h3><?= __('Messagelogs') ?></h3>
    <table cellpadding="0" cellspacing="0">
        <thead>
            <tr>
                <th><?= $this->Paginator->sort('id') ?></th>
                <th><?= $this->Paginator->sort('model_name') ?></th>
                <th><?= $this->Paginator->sort('modelid') ?></th>
                <th><?= $this->Paginator->sort('status') ?></th>
                <th><?= $this->Paginator->sort('deleted') ?></th>
                <th><?= $this->Paginator->sort('created') ?></th>
                <th><?= $this->Paginator->sort('modified') ?></th>
                <th class="actions"><?= __('Actions') ?></th>
            </tr>
        </thead>
        <tbody>
            <?php foreach ($messagelogs as $messagelog): ?>
            <tr>
                <td><?= $this->Number->format($messagelog->id) ?></td>
                <td><?= h($messagelog->model_name) ?></td>
                <td><?= $this->Number->format($messagelog->modelid) ?></td>
                <td><?= $this->Number->format($messagelog->status) ?></td>
                <td><?= $this->Number->format($messagelog->deleted) ?></td>
                <td><?= $this->Number->format($messagelog->created) ?></td>
                <td><?= $this->Number->format($messagelog->modified) ?></td>
                <td class="actions">
                    <?= $this->Html->link(__('View'), ['action' => 'view', $messagelog->id]) ?>
                    <?= $this->Html->link(__('Edit'), ['action' => 'edit', $messagelog->id]) ?>
                    <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $messagelog->id], ['confirm' => __('Are you sure you want to delete # {0}?', $messagelog->id)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
    </table>
    <div class="paginator">
        <ul class="pagination">
            <?= $this->Paginator->prev('< ' . __('previous')) ?>
            <?= $this->Paginator->numbers() ?>
            <?= $this->Paginator->next(__('next') . ' >') ?>
        </ul>
        <p><?= $this->Paginator->counter() ?></p>
    </div>
</div>
