...
<div class="subjects index large-9 medium-8 columns content">
    <h3><?= __($classrooms->name." - ".$classrooms->section) ?></h3>
    <?= $this->Form->create($classrooms, ['type' => 'file']) ?>
        ...
        <table cellpadding="0" cellspacing="0">
            ...
            <tbody>
                <?php for($i=1;$i<=$settings[4]->value;$i++){?>
                    <tr>
                        <?php if($i==$settings[0]['value']){ ?>
                        <td>
                            <?php echo $i;?>
                            <input type="radio" name="halfday" value="<?php echo $i;?>" checked="checked"/> Lunch
                        </td>
                        <?php }else{?>
                            <td>
                                <?php echo $i;?>
